  ...
  <view>
    ...
    <view class="match-info">
      ...
      <u-row class="quarter-item" v-for="(row, index) in (periodGoals.rows||[])" :key="index">
        <u-col span="4">
          <u-row>
            <u-col span="4">
              <u-image mode="aspectFit" width="100%" height="52"
                       :src="index ? teamInfo.rightBadge : teamInfo.leftBadge"></u-image>
            </u-col>
            <u-col span="8">
    ...
    <view class="match-info" v-for="(team, key) in playerStats" :key="key">
      ...
      <scroll-view class="stat-scroll" scroll-x="true">
        <view class="stat-head">
          <view class="stat-row">
            <text class="stat-col player-stat-item" v-for="(item, i) in getStatHeadScroll(key)" :key="i">{{
                item
              }}
            </text>
          </view>
        ...
        <view class="stat-head">
          <view class="stat-row" v-for="(row, r) in getStatBody(key)" :key="r">
            <text class="stat-col player-stat-item" v-for="(item, i) in getStatBodyScroll(row)" :key="i">{{
                item
              }}
            </text>
          </view>
...
export default {
  ...
  computed: {
    videoUrl() {
      // return this.index ? this.video : this.video3;
